...
{
	if (auto cannon = manager_->Create<StaticObject>(static_cast<uint8>(Model::Camp::CANNON)))
	{
	...
	if (auto barrel = manager_->Create<StaticObject>(static_cast<uint8>(Model::Camp::CANNON_BARREL)))
	{
	...
	if (auto bullet = manager_->Create<StaticObject>(static_cast<uint8>(Model::Camp::CANNON_BULLET)))
	{
