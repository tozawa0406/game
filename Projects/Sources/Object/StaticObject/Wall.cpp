...
void Wall::Init(void)
{
	...
	// 手前左
	if (auto stone = manager_->Create<StaticObject>(static_cast<uint8>(Model::Camp::ROCK_1)))
	{
		stone->SetTransform(Transform(VECTOR3(-80, 0, -80), VECTOR3(0), VECTOR3(2)));
		stone->SetColliderOffset(VECTOR3(0), VECTOR3(0), VECTOR3(30));
	}
	// 手前正面
	if (auto stone = manager_->Create<StaticObject>(static_cast<uint8>(Model::Camp::ROCK_14)))
	{
	...
	// 手前右
	if (auto stone = manager_->Create<StaticObject>(static_cast<uint8>(Model::Camp::ROCK_12)))
	{
	...
	// 手前右真ん中
	if (auto stone = manager_->Create<StaticObject>(static_cast<uint8>(Model::Camp::ROCK_4)))
	{
