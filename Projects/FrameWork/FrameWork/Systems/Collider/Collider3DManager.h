...
public:
	OBB(void)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			normaDirect_[i] = Float3(0, 0, 0);
			length_[i] = 0;
		}
	}

	// set
	void SetPos(Float3 pos) { this->pos_ = pos; }
	void SetDirect(int i, Vector3 direct) { this->normaDirect_[i] = direct.GetNorm(); }
	void SetLength(int i, float length) { this->length_[i] = length; }

	// 指定軸番号の方向ベクトルを取得
	Vector3 GetDirect(int i) { return this->normaDirect_[i]; }
	// 指定軸方向の長さを取得
	float   GetLen(int i) { return this->length_[i]; }
	// 位置を取得
	Float3  GetPos(void) { return this->pos_; }
};
...
//	friend Collider3D::Segment;
//	friend Collider3D::Sphere;
