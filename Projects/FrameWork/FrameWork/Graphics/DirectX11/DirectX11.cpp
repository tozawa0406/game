...
HRESULT DirectX11::InitAll(void)
{
	...
	{
		wrapper_->Init();
		static_cast<Dx11Wrapper*>(wrapper_)->GetFont()->CreateStringData("A阿あ", 100);
	}
...
{
	static_cast<Dx11Wrapper*>(wrapper_)->GetFont()->Draw("あ阿", VECTOR2((float)Half(Windows::WIDTH), (float)Half(Windows::HEIGHT)), VECTOR2(100, 100), COLOR(0, 0, 0, 1));
